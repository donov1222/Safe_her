...
/**
 * Clase principal del sistema Smartfut.
 * Coordina la interaccion entre Usuario, Administrador, Cancha, Horario,
 * Reserva, Pago y Notificacion. Contiene el metodo main().
 *
 * Nota: se usan arreglos normales con un contador (en vez de ArrayList/List)
 * y ciclos for/while con indice (en vez de for-each), tal como lo pide la practica.
 */
public class Smartfut {
    ...
    // ---------- GETTERS Y SETTERS usados por la clase Administrador ----------
    ...
    public void iniciarSistema() {
        ...
        // Canchas iniciales (segun el boceto del documento)
    ...
    private void menuReservas() {
        if (usuarioActual == null) {
            JOptionPane.showMessageDialog(null, "Debe iniciar sesion primero (Menu Usuario).");
        } else {
            String texto = "=== RESERVAR CANCHA ===\nCanchas disponibles:\n";
            int i = 0;
            while (i < cantidadCanchas) {
                texto = texto + listaCanchas[i].getIdCancha() + ". " + listaCanchas[i].getNombre()
            ...
            if (entradaId != null) {
                ...
                if (canchaSeleccionada == null) {
                    JOptionPane.showMessageDialog(null, "No existe una cancha con ese ID.");
                } else {
                    String fecha = JOptionPane.showInputDialog(null, "Fecha (dd/mm/aaaa):");
                    String horaInicio = JOptionPane.showInputDialog(null, "Hora de inicio (ej. 18:00):");
                    String horaFin = JOptionPane.showInputDialog(null, "Hora de fin (ej. 19:00):");

                    if (fecha != null && horaInicio != null && horaFin != null) {
                        Horario nuevoHorario = new Horario(fecha, horaInicio, horaFin);

                        if (horarioDisponible(idCancha, nuevoHorario)) {
                            Reserva nuevaReserva = new Reserva(cantidadReservas + 1, usuarioActual,
                                    canchaSeleccionada, nuevoHorario);
                            ...
                            registrarNotificacion("Su reserva #" + nuevaReserva.getIdReserva()
                                    + " fue creada. Recuerde completar el pago.", "Reserva");
                        } else {
                            JOptionPane.showMessageDialog(null, "Ese horario ya esta ocupado para esa cancha.");
                        }
                    }
                }
            }
    ...
    // Revisa que el horario no choque con otra reserva activa de la misma cancha
    private boolean horarioDisponible(int idCancha, Horario horario) {
        ...
        while (i < cantidadReservas) {
            ...
            boolean estaActiva = !r.getEstado().equals("Cancelada");
    ...
    private void menuPagos() {
        if (usuarioActual == null) {
            ...
            while (i < cantidadReservas) {
                Reserva r = listaReservas[i];
                boolean esDelUsuario = r.getUsuario().getIdUsuario() == usuarioActual.getIdUsuario();
            ...
            if (cantidadPendientes == 0) {
                JOptionPane.showMessageDialog(null, "No tiene reservas pendientes de pago.");
            } else {
                ...
                if (entradaId != null) {
                    ...
                    if (reserva == null || !reserva.getEstado().equals("Pendiente")) {
                        JOptionPane.showMessageDialog(null, "Esa reserva no existe o ya fue pagada.");
                    } else {
                        String metodoTexto = "=== PAGO DE RESERVA ===\n"
                                + "Monto a pagar: c" + reserva.getCancha().getPrecioHora() + "\n"
                                + "Seleccione metodo de pago:\n"
                                + "1. Efectivo\n"
                                + "2. Transferencia\n"
                                + "3. Tarjeta de credito";

                        String entradaMetodo = JOptionPane.showInputDialog(null, metodoTexto);
                        if (entradaMetodo != null) {
                            ...
                            Pago nuevoPago = new Pago(cantidadPagos + 1, reserva.getCancha().getPrecioHora(),
                                    metodoPago, reserva);
                            ...
                            registrarNotificacion("Su pago de la reserva #" + reserva.getIdReserva()
                                    + " fue confirmado.", "Pago");
                        }
                    }
                }
    ...
    private void menuAdministrador() {
        String usuarioIngresado = JOptionPane.showInputDialog(null, "Usuario administrador:");
        String contrasenaIngresada = JOptionPane.showInputDialog(null, "Contrasena:");

        if (usuarioIngresado != null && contrasenaIngresada != null
                && admin.validarCredenciales(usuarioIngresado, contrasenaIngresada)) {
            ...
            while (!volver) {
                ...
                if (entrada == null) {
                    ...
                    if (opcion == 1) {
                        ...
                        JOptionPane.showMessageDialog(null, admin.gestionarReservas(this));
                    } else if (opcion == 4) {
                        generarReporte();
                    } else if (opcion == 5) {
                        volver = true;
        ...
        } else if (usuarioIngresado != null && contrasenaIngresada != null) {
            JOptionPane.showMessageDialog(null, "Usuario o contrasena incorrectos.");
        }
    }

    private void agregarCanchaDesdeMenu() {
        String entradaId = JOptionPane.showInputDialog(null, "ID de la nueva cancha:");
        String nombre = JOptionPane.showInputDialog(null, "Nombre de la cancha:");
        String tipo = JOptionPane.showInputDialog(null, "Tipo (Futbol 5 / Futbol 7 / Futbol 11):");
        String entradaPrecio = JOptionPane.showInputDialog(null, "Precio por hora:");

        if (entradaId != null && nombre != null && tipo != null && entradaPrecio != null) {
            int id = leerEntero(entradaId);
            double precio = leerDecimal(entradaPrecio);
            Cancha nueva = new Cancha(id, nombre, tipo, precio, true);
            String resultado = admin.agregarCancha(this, nueva);
            JOptionPane.showMessageDialog(null, resultado);
        }
    }

    private void eliminarCanchaDesdeMenu() {
        String entradaId = JOptionPane.showInputDialog(null, "ID de la cancha a eliminar:");
        if (entradaId != null) {
            int id = leerEntero(entradaId);
            String resultado = admin.eliminarCancha(this, id);
            JOptionPane.showMessageDialog(null, resultado);
        }
    }
    ...
    public void generarReporte() {
        ...
        while (i < cantidadPagos) {
            if (listaPagos[i].getEstadoPago().equals("Confirmado")) {
                totalRecaudado = totalRecaudado + listaPagos[i].getMonto();
            }
            i = i + 1;
        }

        String reporte = "=== REPORTE SMARTFUT ===\n"
                + "Usuarios registrados: " + cantidadUsuarios + "\n"
                + "Canchas registradas: " + cantidadCanchas + "\n"
                + "Total de reservas: " + cantidadReservas + "\n"
    ...
    private void registrarNotificacion(String mensaje, String tipo) {
        if (cantidadNotificaciones < listaNotificaciones.length) {
            Notificacion nueva = new Notificacion(mensaje, "Hoy", tipo);
    ...
    // =========================================================
    // METODOS DE APOYO PARA LEER NUMEROS DESDE JOptionPane
    // =========================================================
